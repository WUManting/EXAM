var tools = {
	...
	getBody : function () {
	 ...
	 /*
			...
			fn <function> 事件处理函数
			isCapture <boolean> 是否捕获，默认 false(IE8以上有效，不含IE8)
	...
	*/
	...
	/*
			事件监听的删除
			obj<DOM object> DOM对象
			type<string> 事件句柄（不带on)
			fn <function> 移除事件的处理函数
			isCapture <boolean> 监听事件在捕获阶段，移除要传true。。。。是否捕获，默认 false(IE8以上有效，不含IE8)
